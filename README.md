...
- 사람의 열체크를 해서 선풍기 세기를 자동 조절

= 열 체크는 열 감지 카메라가 있어야 하므로 불가능

- 센서를 통해 사람이 가까이 오면 동작, 멀어지면 자동 종료

= 이는 초음파 센서를 활용할 예정

- 블루투스,  wifi  를 이용해서 선풍기 간의 통신?

= 챗봇을 통하여 gpio를 조절하여 수정할 예정(포트포워딩으로 인해 불가)

- 휴대폰으로 바람세기 조절, 시간 조절..

= 이도 챗봇을 통해 해결되는 문제(포트포워딩으로 인해 불가)

## 소프트웨어 기능 리스트

- 초음파 거리 계산
- 거리 50cm 이내로 감지되면 모터 작동
